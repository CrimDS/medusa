...
/* zlib 1.3.2 added _z helper functions that compare arguments against NULL,
   but zlib.h only defines Z_NULL (=0).  Under Z_SOLO builds (used by Medusa
   for the file-I/O-free configuration in Medusa.vcxproj), zutil.h also
   skips <stddef.h>, leaving NULL undefined here.  Pulled in directly so
   this TU compiles regardless of Z_SOLO state — <stddef.h> is freestanding
   (brings NULL / size_t / ptrdiff_t with no file-I/O baggage). */
#include <stddef.h>
